...
//echo "$artist, $song";
$exists = $proxy->checkSongExists($artist, $song);
if ($exists == 1)
	$result = $proxy->getSong($artist, $song);

//$result = $proxy->searchArtists($artist);
...
if ($proxy->fault) {
	...
	if ($err) {
		...
		// Display the result
		$ex = explode("</lyric",$result['lyrics']);
		if (trim($ex[0]) != "") echo $result['song']."\n".
							 $result['artist']."\n".
							 "\n".
							 trim($ex[0])."\n"."\n".
							 "[ Get Lyrical is powered by LyricWiki.org : ".$result['url']." ]";
	}
}
/*
